...
class BertBilstmClassifier(nn.Module):
    def __init__(self, model_name, freeze_bert=True):
        super(BertBilstmClassifier, self).__init__()
        # Specify hidden size of BERT, hidden size of our classifier, and number of labels
        D_in, H, D_out = 768, 128, 3
    ...
    def forward(self, input_ids, attention_mask):
        ...
        output_lstm =  self.bilstm(bert_last_hidden_state)
        # Extract the last hidden state of the token `[CLS]` for classification task
        lstm_last_hidden_state_cls = output_lstm[0][:, 0, :]

        # Feed input to classifier to compute logits
        logits = self.classifier(lstm_last_hidden_state_cls)
...
if __name__ == "__main__":
    ...
    model = BertBilstmClassifier(model_name=model_name,freeze_bert=True).to(device)
