...
if __name__ == "__main__":
    ...
    arg("--predict", "-p", help="Prediction column",  default='Экстраверсия') #required=True)
    arg("--lr",  type=float, help="Learning Rate", default=2e-5)
    ...
    dataset = pd.read_csv('../df_tokens_ocean.csv')
    dataset['lengths'] = dataset.tokens.apply(lambda x: len(str(x).split()))
    df = dataset.query('lengths > @minl')[['tokens', prediction_column]]
    df[prediction_column] = df[prediction_column].apply(lambda x: int(x)-1)
    ...
    train_data = pd.concat([train_data,
                    train_data.query('Экстраверсия == 0'),
                    train_data.query('Экстраверсия == 0'),
                    train_data.query('Экстраверсия == 4'),
                    train_data.query('Экстраверсия == 4'),
                    train_data.query('Экстраверсия == 4'),
                    train_data.query('Экстраверсия == 4')])
    ...
    torch.save(model.state_dict(),f'e_bert_overs.bin')
